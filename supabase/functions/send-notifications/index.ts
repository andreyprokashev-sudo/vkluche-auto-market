...
function maxActions(type:string,auctionId?:string,listingId?:string,questionId?:string,auctionStatus?:string,inquiryId?:string){
  if(type==='credit_status')return[{type:'inline_keyboard',payload:{buttons:[[{type:'link',text:'Открыть кредитные заявки',url:'https://andreyprokashev-sudo.github.io/vkluche-auto-market/?cabinet=my-credit'}]]}}]
  if(type==='history_report_requested')return[{type:'inline_keyboard',payload:{buttons:[[{type:'link',text:'Прикрепить отчёт',url:'https://andreyprokashev-sudo.github.io/vkluche-auto-market/?cabinet=seller-inquiries'}]]}}]
  if(type==='history_report_ready')return[{type:'inline_keyboard',payload:{buttons:[[{type:'link',text:'Скачать отчёт',url:`https://andreyprokashev-sudo.github.io/vkluche-auto-market/?cabinet=my-inquiries${inquiryId?`&inquiry=${inquiryId}`:''}`}]]}}]
  const listingLink=questionId?`https://andreyprokashev-sudo.github.io/vkluche-auto-market/?cabinet=questions&question=${questionId}`:`https://andreyprokashev-sudo.github.io/vkluche-auto-market/?listing=${listingId||''}`
...
async function sendMax(chatId:string,subject:string,body:string,type:string,auctionId?:string,listingId?:string,questionId?:string,auctionStatus?:string,inquiryId?:string){
  if(!maxToken)throw new Error('MAX_BOT_TOKEN не настроен')
  const attachments=maxActions(type,auctionId,listingId,questionId,auctionStatus,inquiryId)
...
async function sendTelegram(chatId:string,subject:string,body:string,type:string,auctionId?:string,listingId?:string,questionId?:string,auctionStatus?:string,inquiryId?:string){
  const link=type==='credit_status'?'https://andreyprokashev-sudo.github.io/vkluche-auto-market/?cabinet=my-credit':type==='history_report_requested'?'https://andreyprokashev-sudo.github.io/vkluche-auto-market/?cabinet=seller-inquiries':type==='history_report_ready'?`https://andreyprokashev-sudo.github.io/vkluche-auto-market/?cabinet=my-inquiries${inquiryId?`&inquiry=${inquiryId}`:''}`:questionId?`https://andreyprokashev-sudo.github.io/vkluche-auto-market/?cabinet=questions&question=${questionId}`:listingId?`https://andreyprokashev-sudo.github.io/vkluche-auto-market/?listing=${listingId}${type==='new_message'?'&chat=1':''}`:'https://andreyprokashev-sudo.github.io/vkluche-auto-market/',button=type==='credit_status'?'Открыть кредитные заявки':type==='history_report_requested'?'Прикрепить отчёт':type==='history_report_ready'?'Скачать отчёт':type==='new_message'?'Открыть переписку':type==='listing_moderation'?'Открыть карточку и результаты':type.startsWith('inspection_')?'Открыть запись на осмотр':type==='question_received'||type==='question_reminder'?'Открыть вопрос':['auction_finished','auction_won','deal_confirmed'].includes(type)?'Открыть результат':'Открыть автомобиль',inline_keyboard:any[]=[[{text:button,url:link}]]
