...
async function send(to:string,subject:string,body:string,idempotency:string){
  const response=await fetch('https://api.resend.com/emails',{method:'POST',headers:{authorization:`Bearer ${resend}`,'content-type':'application/json','idempotency-key':idempotency},body:JSON.stringify({from,to:[to],subject,html:`<div style="font-family:Arial,sans-serif;max-width:580px;margin:auto"><div style="display:flex;align-items:center;gap:10px"><img src="https://andreyprokashev-sudo.github.io/vkluche-auto-market/assets/brand/vkluche-logo-wheel.png" width="48" height="48" alt="ВКЛЮЧЕ" style="border-radius:12px"><h2 style="color:#174caa">ВКЛЮЧЕ</h2></div><h3>${esc(subject)}</h3><p style="line-height:1.6;color:#475467">${esc(body)}</p><p><a href="https://andreyprokashev-sudo.github.io/vkluche-auto-market/">Открыть ВКЛЮЧЕ →</a></p></div>`,text:`${subject}\n\n${body}\n\nhttps://andreyprokashev-sudo.github.io/vkluche-auto-market/`})})
