...
function mapAd(ad: Record<string, any>) {
  ...
  const body = text(ad.BodyType), condition = text(ad.Condition).toLowerCase(), images = imageUrls(ad)
  ...
  return {
    ...
    car: {
      ...
      city, date: 'из фида', type: [condition.includes('нов') ? 'new' : 'used', /внедорож|кроссов/i.test(body) ? 'suv' : ''].filter(Boolean),
      ...
      details: { brand, model, body, condition, gearbox: text(ad.Transmission), drive: text(ad.DriveType), color: text(ad.Color), description: text(ad.Description), seller: text(ad.ManagerName || ad.ContactName), phone: text(ad.ContactPhone), images }
    }
  }
}
