(function () {
  ...
  function showChannelOnboarding(waitingForEmail=false){showView('channels');const button=document.querySelector('#registrationMaxConnect'),text=document.querySelector('#channelOnboardingText');button.disabled=waitingForEmail;button.textContent=waitingForEmail?'После подтверждения почты':'Подключить';text.textContent=waitingForEmail?'Аккаунт создан. Подтвердите почту, войдите — и мы сразу предложим подключить MAX.':'Подключите мессенджер, чтобы не пропустить начало торгов, перебитую ставку и ответ продавца.';localStorage.setItem('vkluche-channel-onboarding','pending')}
