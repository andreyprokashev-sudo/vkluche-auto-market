(function () {
  ...
  function availableTabs() {
    ...
    if (state.accountType === "professional" || state.role === "admin") {
      const companyTabs=[["business", "Обзор компании"],["business-inventory", "Склад"],["business-auctions", "Массовые торги"],["business-feeds", "Фиды"],["business-team", "Филиалы и сотрудники"]];
  ...
  content.addEventListener("click", async (event) => {
    ...
    if(event.target.closest('[data-dashboard-help]')){document.querySelector('#authModal [data-auth-close]')?.click();await window.vklucheHelp?.open?.();window.vklucheHelp?.show?.(state.active==='deals'?'deal':state.active==='reports'?'reports':state.active==='business-analytics'?'analytics':'dashboard');return}
