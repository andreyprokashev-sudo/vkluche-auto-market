(function () {
  ...
  let state = { role: "user", accountType: "private", active: "overview", listingFilter: "all", inventorySearch: "", reportPeriod: "30", data: {} };
  const esc = (value) =>
  ...
  content.addEventListener("click", async (event) => {
    ...
    if(event.target.closest('[data-dashboard-help]')){document.querySelector('#authModal [data-auth-close]')?.click();await window.vklucheHelp?.open?.();window.vklucheHelp?.show?.(state.active==='deals'?'deal':state.active==='reports'?'reports':'dashboard');return}
