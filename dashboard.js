(function () {
  ...
  function availableTabs() {
    ...
    if (state.accountType === "professional" || state.role === "admin") {
      ...
      if(state.role==='admin'||['owner','administrator','manager'].includes(state.data.organizationRole))companyTabs.push(["business-analytics", "Аналитика"]);
  ...
  content.addEventListener("click", async (event) => {
    ...
    if(event.target.closest('[data-dashboard-help]')){document.querySelector('#authModal [data-auth-close]')?.click();await window.vklucheHelp?.open?.();window.vklucheHelp?.show?.(state.active==='deals'?'deal':state.active==='reports'?'reports':state.active==='business-analytics'?'analytics':state.active==='market-analytics'?'marketPrice':state.active==='inventory-recommendations'?'inventoryAdvice':state.active==='participant-funnel'?'participantFunnel':'dashboard');return}
