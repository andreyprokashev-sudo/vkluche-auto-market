(function () {
  ...
  function renderAdminUsers(){if(state.data.adminUsers===undefined&&!state.data.adminUsersLoading){loadAdminUsers();content.innerHTML=`${pageHead('Пользователи','Управление доступом, ролями и компаниями','<button type="button" data-dashboard-help>Как работать?</button>')}<div class="dashboard-loading">Загружаем пользователей…</div>`;return}const users=state.data.adminUsers||[],query=state.adminUserSearch.trim().toLowerCase(),now=Date.now(),isBlocked=user=>user.banned_until&&new Date(user.banned_until).getTime()>now,filtered=users.filter(user=>{const matches=!query||[user.name,user.email,user.phone,user.city,...(user.organizations||[]).map(item=>item.organizations?.name)].some(value=>String(value||'').toLowerCase().includes(query));const group=state.adminUserFilter==='blocked'?isBlocked(user):state.adminUserFilter==='admins'?user.role==='admin':state.adminUserFilter==='professional'?user.account_type==='professional':true;return matches&&group}),organizations=state.data.adminUserOrganizations||[];content.innerHTML=`${pageHead('Пользователи','Блокировка, права и принадлежность к компаниям','<button type="button" data-dashboard-help>Как работать?</button>')}<div class="dashboard-stats">${stat(users.length,'зарегистрировано')}${stat(users.filter(isBlocked).length,'заблокировано')}${stat(users.filter(user=>user.role==='admin').length,'администраторов')}${stat(users.filter(user=>user.account_type==='professional').length,'профессиональных')}</div><div class="admin-user-tools"><input type="search" data-admin-user-search value="${esc(state.adminUserSearch)}" placeholder="Имя, почта, телефон или компания"><div>${[['all','Все'],['professional','Профессиональные'],['admins','Администраторы'],['blocked','Заблокированные']].map(([value,label])=>`<button type="button" class="${state.adminUserFilter===value?'active':''}" data-admin-user-filter="${value}">${label}</button>`).join('')}</div></div>${state.data.adminUsersError?`<div class="market-note"><b>Ошибка загрузки</b><span>${esc(state.data.adminUsersError)}</span></div>`:''}<section class="admin-user-list">${filtered.map(user=>{const blocked=isBlocked(user),memberships=user.organizations||[];return`<article class="admin-user-card ${blocked?'blocked':''}" data-admin-user-id="${user.id}"><header><span class="admin-user-avatar">${esc((user.name||user.email||'?').slice(0,1).toUpperCase())}</span><div><b>${esc(user.name||'Имя не указано')}</b><small>${esc(user.email||'Почта не указана')} · зарегистрирован ${date(user.created_at)}</small><small>${user.last_sign_in_at?`Последний вход ${date(user.last_sign_in_at)}`:'Ещё не входил'} · ${user.email_confirmed_at?'почта подтверждена':'почта не подтверждена'}</small></div><em class="${blocked?'blocked':''}">${blocked?'Заблокирован':user.role==='admin'?'Администратор':'Активен'}</em></header><div class="admin-user-memberships">${memberships.map(item=>`<span><b>${esc(item.organizations?.name||'Компания')}</b><small>${esc(item.member_role)}</small>${item.member_role!=='owner'?`<button type="button" data-admin-remove-org="${item.organization_id}" data-user-id="${user.id}">×</button>`:''}</span>`).join('')||'<small>Не состоит в компании</small>'}</div><div class="admin-user-actions"><label>Тип аккаунта<select data-admin-account-type="${user.id}"><option value="private"${user.account_type==='private'?' selected':''}>Частное лицо</option><option value="professional"${user.account_type==='professional'?' selected':''}>Профессиональный</option></select></label><label>Системная роль<select data-admin-role="${user.id}"><option value="user"${user.role!=='admin'?' selected':''}>Пользователь</option><option value="admin"${user.role==='admin'?' selected':''}>Администратор</option></select></label><button type="button" class="${blocked?'restore':'danger'}" data-admin-block-user="${user.id}" data-blocked="${blocked}">${blocked?'Разблокировать':'Заблокировать'}</button></div>${organizations.length?`<form data-admin-add-org="${user.id}"><select name="organizationId" required><option value="">Добавить в компанию…</option>${organizations.map(org=>`<option value="${org.id}">${esc(org.name)}</option>`).join('')}</select><select name="memberRole"><option value="viewer">Наблюдатель</option><option value="manager">Менеджер</option><option value="administrator">Администратор компании</option></select><button type="submit">Добавить</button></form>`:''}</article>`}).join('')||empty('По заданным условиям пользователи не найдены.')}</section>`}
