(function () {
  ...
  let state = { role: "user", accountType: "private", active: "overview", listingFilter: "all", inventorySearch: "", reportPeriod: "30", analyticsPeriod: "90", funnelPeriod: "90", data: {} };
  const esc = (value) =>
  ...
  content.addEventListener("click", async (event) => {
    ...
    if(event.target.closest('[data-dashboard-help]')){document.querySelector('#authModal [data-auth-close]')?.click();await window.vklucheHelp?.open?.();window.vklucheHelp?.show?.(state.active==='deals'?'deal':state.active==='reports'?'reports':state.active==='business-analytics'?'analytics':state.active==='market-analytics'?'marketPrice':state.active==='inventory-recommendations'?'inventoryAdvice':state.active==='participant-funnel'?'participantFunnel':state.active==='business-integrations'?'integrations':'dashboard');return}
